...
StrataGraph turns a codebase into a single queryable graph and answers one question precisely: **what breaks if I change this?** It crosses the boundaries that ordinary code-intelligence tools stop at: code, API contracts, cloud infrastructure, the database, and the repository's own documentation. So the answer includes the Lambda that implements a GraphQL field, the frontend in another repository that queries it, the table a handler reads, and the doc sections the change puts at risk, not just the functions in the file you are editing. Every relationship it reports carries provenance and a calibrated confidence, and where it cannot be sure it says so rather than guessing.
...
- **Ask the repository's own knowledge**: a token-budgeted `guidance` digest for any symbol or file (its doc comment, plus the docs that mention it, bodies read live from disk), lexical `search-docs` over the whole docs tree, and doc drift measured — stale docs surface in every blast radius as `needs review`. ([The knowledge plane](concepts/knowledge.md))
